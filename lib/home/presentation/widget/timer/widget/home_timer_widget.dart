...
import 'package:maeum_ga_gym_flutter/home/presentation/widget/timer/widget/home_timer_custom_painter.dart';
import 'package:maeum_ga_gym_flutter/home/presentation/widget/timer/widget/timer_text_format.dart';
...
class HomeTimerWidget extends StatefulWidget {
  ...
  */
  ...
  @override
  HomeTimerWidgetState createState() => HomeTimerWidgetState();
}

class HomeTimerWidgetState extends State<HomeTimerWidget>
    with TickerProviderStateMixin {
  AnimationController? _controller;
  Animation<double>? _countDownAnimation;
  HomeTimerController? countDownController;
  ...
  String _getTime(Duration duration) {
    // For HH:mm:ss format
    if (widget.textFormat == TimerTextFormat.HH_MM_SS) {
      return '${duration.inHours.toString().padLeft(2, '0')}:${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For mm:ss format
    else if (widget.textFormat == TimerTextFormat.MM_SS) {
      return '${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For ss format
    else if (widget.textFormat == TimerTextFormat.SS) {
      return (duration.inSeconds).toString().padLeft(2, '0');
    }
    // For s format
    else if (widget.textFormat == TimerTextFormat.S) {
  ...
  @override
  void initState() {
    countDownController = widget.controller ?? HomeTimerController();
    super.initState();
              ...
              child: AspectRatio(
                ...
                child: Stack(
                  children: <Widget>[
                    Positioned.fill(
                      child: CustomPaint(
                        painter: HomeTimerCustomPainter(
